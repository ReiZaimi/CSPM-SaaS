...
AWS is implemented behind the same connector seam — connector, IAM manifest, onboarding, change events, thirty rules, CIS AWS 3.0 (28 of 31 controls) — and **has never been run against a live AWS account**. It is reachable through the API and gated out of the UI by `AWS_ENABLED` until `docs/AWS_INTEGRATION.md` §1's checklist passes. Treat every string in `app/connectors/aws/` as unverified until then.
